function [ Likelihood ] = Hypothesis_Test2( DATA,bound_vector, I, epsilon, T, N, n, n_add_one, Para_num)
% This is a subroutine program to calculate the likelihood under beta = [0;0].
% Input: 
...
%       for K simulated datasets with beta = [0;0].
    X = DATA(:,2:n-1);
    ...
    A_beta = [-1,0;1,0;0,-1;0,1;];b_beta=[-0.00000001;10;-0.00000001;10];
    lambda_hat = Initial_Points(1,1); alpha_hat = Initial_Points(1,2);
    beta_hat = [0;0]; theta_hat = Initial_Points(1,n_add_one:Para_num)';

gamma_hat = Efunction( DATA,N,n,lambda_hat,alpha_hat,beta_hat,theta_hat );

[Para_estimator,~] = fmincon(@(Para)Lxv_nonfix( DATA,n,gamma_hat,[Para;beta_hat] ),[lambda_hat;alpha_hat],A_beta,b_beta);
[theta_estimator,~] = fmincon(@(theta)Lz( X,N,gamma_hat,theta ),theta_hat,A_theta,b_theta);
a = [Para_estimator;beta_hat;theta_estimator];
...
log_Likelihood_output = -Negative_log_Likelihood_Calcalator( DATA, [Para_estimator;beta_hat], theta_estimator, N, n);


    
 for m = 2:I
lambda_hat = Initial_Points(m,1); alpha_hat = Initial_Points(m,2);
beta_hat = [0;0]; theta_hat = Initial_Points(m,n_add_one:Para_num)';
...
temp_log_Likelihood_output  = -Negative_log_Likelihood_Calcalator( DATA, [Para_estimator;beta_hat], theta_estimator, N, n);
